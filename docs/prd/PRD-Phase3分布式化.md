...
> 类型：新需求（Requirement）｜状态：部分达成（与《项目计划》口径对齐，2026-08-29 更正，原误标「已完成」）｜对应《项目计划》Phase 3 分布式化。
>
> 达成口径：事件总线（Redis Pub/Sub）与多实例 WebSocket 解耦自 C-170 起在编排中默认启用；断线重连状态对账（`workspace.snapshot` 消费）已于 2026-08-29 补上。**仍缺**：配额限流为单实例内存计数，多实例下失效（待 Redis 固定窗口/令牌桶改造，见差距审查报告 §2.3）。
