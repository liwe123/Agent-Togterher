...
> 类型：新需求（Requirement）｜状态：已完成（2026-08-20 上线，C-137 / 251e387；群聊 @外部节点路由 + Cursor 文件桥真实执行已落地）｜目标：让群聊 `@Cursor` 能真正派活到 Cursor 集成节点，并通过宿主文件系统桥接让 Cursor IDE 受控产出 `output.md` 回写群聊；补齐「外部 Agent 软件接入」（C-118 / C-129）当前缺失的路由与真实执行链路。
...
## 16. 实施状态（已完成，2026-08-20 回写）

- 方案 B（文件桥，Docker 友好）已按 §5 落地，见文首状态行：C-137 / `251e387`。
- FR1–FR10 全部实施完成（群聊 @ 路由 + Cursor 文件桥真实执行，C-137/C-138）。
- 历史备注：本节原为「方案已定，待落地」的施工前记录，C-137 上线后未及时回写；
  2026-09-25 按代码现状修正，正文其余章节为方案存档，不再代表未落地。
